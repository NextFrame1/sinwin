...
async def top_workers_callback(call: CallbackQuery):
	...
	messages = [
				...
				' 🥇 WOT****3156: 1 000 000 рублей',
				'🥈 RNP****6871: 800 000 рублей',
				'🥉 RNP****8165: 700 000 рублей',
				'🏅 WOK****2899: 600 000 рублей',
				'🏅 WOI****9437: 450 000 рублей\n',
				'👽️ Ваша позиция в топе: 10\n',
				'Статистика обнуляется 1 числа каждого месяца. <code>Пользователь который занял первое место переходит на следующий статус, если его текущий статус "Профессионал" или ниже.</code>\n',
				'<code>Второе и третье место получат премию.</code>'
	]
...
async def status_callback(call: CallbackQuery):
	...
	messages = ['🏆️ Ваш текущий статус: Новичок', '🎯 Вы получаете: 35%\n', '📊 Ваш доход за последний месяц: 15 000 RUB',
				...
				'<code>Обратите внимание: условия перехода могут меняться.</code>', 'Если у вас есть вопросы, напишите в поддержку']
